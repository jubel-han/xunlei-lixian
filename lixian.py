...
class XunleiClient(object):
    ...
    def set_cookie(self, domain, k, v):
        c = cookielib.Cookie(version=0, name=k, value=v, port=None, port_specified=False, domain=domain,
    ...
    def login(self, username=None, password=None):
        ...
        login_page = self.urlopen('http://login.xunlei.com/sec2login/',
                                  data={'u': username, 'p': password, 'verifycode': verification_code})
        self.id = self.get_userid()
        with self.attr(page_size=1):
            login_page = self.urlopen(
                'http://dynamic.lixian.vip.xunlei.com/login?cachetime=%d&from=0' % current_timestamp()).read()
    ...
    def to_page_url(self, type_id, page_index, page_size):
        ...
        url = 'http://dynamic.cloud.vip.xunlei.com/interface/showtask_unfresh?type_id=%s&page=%s&tasknum=%s&p=%s&interfrom=task' % (
        type_id, page, page_size, p)
        return url
    ...
    def read_all_tasks_on_demand(self, type_id):
        ...
        fetch_page = lambda page_index, page_size: self.read_task_page_info_by_page_index(type_id, page_index,
                                                                                          page_size)
    ...
    def read_history_page_url(self, url):
        self.set_cookie('.vip.xunlei.com', 'lx_nf_all', urllib.quote(
            'page_check_all=history&fltask_all_guoqi=1&class_check=0&page_check=task&fl_page_id=0&class_check_new=0&set_tab_status=11'))
        page = self.urlread(url).decode('utf-8', 'ignore')
        if not self.has_gdriveid():
    ...
    def list_bt(self, task):
        assert task['type'] == 'bt'
        url = 'http://dynamic.cloud.vip.xunlei.com/interface/fill_bt_list?callback=fill_bt_list&tid=%s&infoid=%s&g_net=1&p=1&uid=%s&noCacheIE=%s' % (
        task['id'], task['bt_hash'], self.id, current_timestamp())
    ...
    def list_vod(self, req_num, req_offset, folder_id=0):

        url = 'http://i.vod.xunlei.com/req_history_play_list/req_num/%d/req_offset/%d?type=kongjian&order=create&t=%s&folder_id=%d' % (
            req_num, req_offset, current_timestamp(), folder_id
        )

        result = json.loads(self.urlread(url).decode('utf-8'))['resp']
        return result['record_num'], result['history_play_list']

    def download_torrents_from_vod(self, folder_id, folder_path):

        if not os.path.exists(folder_path):
            os.mkdir(folder_path)

        total = self.list_vod(0, 0, folder_id)[0]
        page = total // 30 if total % 30 == 0 else total // 30 + 1
        for i in range(0, page):
            result = self.list_vod(30, i * 30, folder_id)

            for task in result[1]:
                sub_folder_id = task.get('folder_id')
                if sub_folder_id:
                    self.download_torrents_from_vod(sub_folder_id, os.path.join(folder_path, task['file_name']))
                    continue
                ...
                task_re = re.search(r'bt%3A//(.+)', url)
                if task_re:
                    task_name = urllib.unquote(str(task['file_name'])).decode('utf-8', 'ignore')
                    task_name = task_name.replace(u'【无效链接】', u'').replace(u'【处理中,请等待】', u'')
                    task_name += u'.torrent'
                    task_path = os.path.join(folder_path,  task_name)
                    if os.path.exists(task_path):
                        continue

                    print task_name.encode('gbk', 'ignore')
                    try:
                        with open(task_path, 'wb') as f:
                            f.write(self.get_torrent_file_by_info_hash(task_re.group(1)))

                        #修改文件日期
                        play_time_str = create_time if not task.get('playtime') else task['playtime']
                        play_time = time.mktime(time.strptime(play_time_str, '%Y-%m-%d %H:%M:%S'))
                        create_time = time.mktime(time.strptime(task['createtime'], '%Y-%m-%d %H:%M:%S'))
                        os.utime(task_path, (play_time, create_time))

                    except Exception, e:
                        print e

    def get_torrent_file_by_info_hash(self, info_hash):
        url = 'http://dynamic.cloud.vip.xunlei.com/interface/get_torrent?userid=%s&infoid=%s' % (
        self.id, info_hash.upper())
    ...
    def add_torrent_task_by_content(self, content, path='attachment.torrent'):
        ...
        upload_success = re.search(r'<script>document\.domain="xunlei\.com";var btResult =(\{.*\});</script>', response,
                                   flags=re.S)
        if upload_success:
    ...
    def add_torrent_task_by_link(self, link, old_task_id=None):
        url = 'http://dynamic.cloud.vip.xunlei.com/interface/url_query?callback=queryUrl&u=%s&random=%s' % (
        urllib.quote(link), current_timestamp())
        response = self.urlread(url)
        success = re.search(r'queryUrl(\(1,.*\))\s*$', response,
                            flags=re.S) # XXX: sometimes it returns queryUrl(0,...)?
        if not success:
    ...
    def delete_tasks_by_id(self, ids):
        ...
        url = 'http://dynamic.cloud.vip.xunlei.com/interface/task_delete?callback=%s&type=%s&noCacheIE=%s' % (
        jsonp, 2, current_timestamp()) # XXX: what is 'type'?
        response = self.urlread(url, data=data)
    ...
    def pause_tasks_by_id(self, ids):
        url = 'http://dynamic.cloud.vip.xunlei.com/interface/task_pause?tid=%s&uid=%s&noCacheIE=%s' % (
        ','.join(ids) + ',', self.id, current_timestamp())
...
def convert_task(data):
    ...
    task = {'id': data['id'],
            ...
            'status_text': {'0': 'waiting', '1': 'downloading', '2': 'completed', '3': 'failed', '5': 'pending'}[
