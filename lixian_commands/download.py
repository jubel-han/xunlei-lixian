...
def verify_mini_hash(path, task):
    return os.path.exists(path) and os.path.getsize(path) == task['size'] and lixian_hash.verify_dcid(path,
                                                                                                      task['dcid'])
...
def download_file(client, path, task, options):
    ...
    if options['node']:
        if options['node'] == 'best' or options['node'] == 'fastest':
            ...
            if task['size'] >= parse_size(options['node_detection_threshold']):
                url = lixian_nodes.use_fast_node(url, options['vod_nodes'],
                                                 parse_size(options['node_detection_acceptable']),
                                                 client.get_gdriveid())
        else:
    ...
    def download1_checked(client, url, path, size):
        ...
        while checked < 10:
            ...
            if download.finished():
                ...
                checked += 1
        assert os.path.getsize(path) == size, 'incorrect downloaded file size (%s != %s)' % (
    ...
    if task['type'] == 'bt':
        ...
        for t in skipped:
            with colors(options.get('colors')).yellow():
                ...
                str(t['id']), t['index'], t['name'].encode(default_encoding), t['status_text'])
        ...
        if not no_hash:
            ...
            verified = lixian_hash_bt.verify_bt(output_path, lixian_hash_bt.bdecode(torrent_file)['info'],
                                                file_set=file_set, progress_callback=bar.update)
            bar.done()
            if not verified:
